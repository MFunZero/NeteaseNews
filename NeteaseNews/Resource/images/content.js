...
function play_video(url,boardcast,vid,commentid,commentboardid,sourceName,element){
    
    var rect = element.getBoundingClientRect();   
    location.href="video:///"+boardcast+"///" + url + "///" + vid + "///" + commentid + "///"+ commentboardid + "///" + encodeURI(sourceName) + "///" + rect.left+ "///" + rect.top + "///" + rect.width+ "///" + rect.height;
}
...
function cancelSuperTouch(){
    var e = window.event;
    //e.cancelBubble = true;
    e.stopPropagation();
}

function simpleTouchStart(){
    this.id = "hover";
}
...
function contentADEvent(){
    ...
    //下载app
    var installes = document.getElementsByClassName('ad_install');
    if(installes && installes.length){
        for(var i = 0; i < installes.length; i++){
            installes[i].addEventListener('touchstart', function(event){
                                          this.id = "hover";
                                          cancelSuperTouch();
                                          }, false);
            installes[i].addEventListener('touchend', function(){
                                          this.id = "";
                                          cancelSuperTouch();
                                          }, false);
            installes[i].addEventListener('click', function(event){
                                          location.href = "plugin:///" + this.getAttribute("url");
                                          cancelSuperTouch();
                                          }, false);
        }
...
function relativeADEvent(tag){
    var machine_ad = document.getElementById('machine_ad_' + tag);
    if(machine_ad){
...
function onLoad(){
    ...
    // 计算并设置 来源显示的最大宽度
    (function(){
        var getCssValue = function(elem,property){
        ...
        var sourceElement = subtitleElement.querySelector(".source"),
        tagsElement = subtitleElement.querySelector(".article-tags"),
        ptimeElement = subtitleElement.querySelector(".ptime"),
        maxWidth = 0;
        if(subtitleElement&&sourceElement&&tagsElement&&ptimeElement){
             maxWidth = parseInt(getCssValue(subtitleElement,"width"))-
             tagsElement.offsetWidth-
             ptimeElement.offsetWidth-
             parseInt(getCssValue(sourceElement,"marginRight"));
             sourceElement.style.maxWidth = maxWidth+"px";
        }
    })();
    ...
    //栏目跳转
    var wraper = document.getElementById('channel_section');
    if(wraper){
        var boxes = wraper.getElementsByTagName('li');
        if(boxes && boxes.length){
            for(var i = 0; i < boxes.length; i++){
                boxes[i].addEventListener('touchstart', function(event){
                                          this.id = "hover";
                                          }, false);
                boxes[i].addEventListener('touchend', function(){
                                          for(var j = 0; j < boxes.length; j++){
                                          boxes[j].id = ""
                                          }}, false);
                boxes[i].addEventListener('click', function(event){
                                          location.href = "plugin:///" + this.firstChild.getAttribute("url");
                                          }, false);
            }
        }
    }
    ...
    //分享按钮
    var wrapper = document.getElementById('share_section');
    if(wrapper){
    ...
    //带的参数是正文底部到屏幕的距离
    setTimeout(function(){location.href = "show://" + document.getElementById('body_section').getBoundingClientRect().bottom;},100);
}
...
function reshow(){
    //带的参数是正文底部到屏幕的距离
    location.href = "reshow://" + document.getElementById('body_section').getBoundingClientRect().bottom;
}
...
{
    var headerHTML = "<div class='section_header'>热门跟贴</div><ul class='section_body'><a id='comment_list' href='comment://' class='no_tc'></a><li><a class='comment_entry no_tc' href='comment://'><div class='more_comment'>查看更多跟贴</div></a></li></ul><div class='section_footer'></div>";

    document.getElementById('comment_section').innerHTML = headerHTML;
}
...
function buildFloors(floors)
{
    ...
    for (var i = 0,len = floors.length; i < len; i++) {
        ...
        var mars    = f["mars"];
        var vip     = f["vip"];
        var authInfo = f["authInfo"];

        buildSingleFloor(icon,mars,name,subname,vote,body,bigv,first,vip,authInfo);
        first = false;
    };
    noFirstPageAD();
}

function buildSingleFloor(usericon,mars,username,subname,vote,body,bigv,first,vip,authInfo)
{
    ...
    var authIcon = "";
    if(authInfo) authIcon = "<i class=\"auth-icon\"></i>";
  
    titleDiv.innerHTML = voteSpan + iconImg + "<div class='floor_username' " + bigvTag + vipTag + ">" + username + authIcon +"</div> <div class='floor_subname'>" + subname + "</div>";
...
function reportButtonClicked(element){
    //如果这个按钮已经是选中状态了就不能点了
    if (element.getAttribute("selected") == 'true') return;
    ...
    location.href = "rate://report/"+ x + "/" + y + "/" + window.pageYOffset + "/" + showWidth + "/" + showHeight;
}

function didFinishReport(){
    var downButton = document.getElementsByClassName("up_down")[0];
    rateButtonClicked(downButton);
}
